...
class PrestamoC extends Controller
{
    //
    function ver(){
        //Obtener los prestamos de la BD
    ...
    function insertar(Request $r)
    {
        ...
        if ($l != null and $l->numEjemplares > 0) {
            ...
            return back()->with('mensaje', 'Error, libro no existe o no hay ejemplares');
        }
    }
    ...
    function actualizar(Request $r,$id){
        ...
        //Chquear si se cambia el libro para
        ...
        //Cambiar los datos del prestamo por los nuevos
            $p->fecha = $r->fecha;
            $p->nombreCliente = $r->cliente;
        ...
        $l = Libro::find($r->libro);
        if($l!=null and $l->numEjemplares>0){
            //Chequear que el cliente no tiene prestamos pendientes
            $pendientes = Prestamo::where('nombreCliente',$r->cliente)->where('fechaDevolucion',null)->get();
            if(sizeof($pendientes)>0){
                return back()->with('mensaje','Error, el cliente tiene libros sin devolver');
            }else{
                //Registrar préstamos y actualizar nº de ejemplares
            ...
            return back()->with('mensaje','Error, libro no existe o no hay ejemplares para prestar');
        }
    }
}
